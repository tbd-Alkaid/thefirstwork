...
class FeedForwardModel(object):
    ...
    def build(self):
        ...
        z_g = tf.zeros(shape=tf.stack([tf.shape(self._dw)[1], 1]), dtype=TF_DTYPE,name='zg')
        for t in range(0, self._num_time_interval-1):
            ...
            z = self._subnetwork(k, str(t + 1))
            # z_g = tf.gradients(z, k)
            self._hessian = [tf.gradients(z[:,i], k)[0] for i in range(self._dim)]###############################
            print(self._hessian)
